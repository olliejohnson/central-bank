...
local changes = {
    { "v0.0.1-alpha", { "Added configuration menu" } }
}
...
local function load_settings(target, raw)
    ...
    local loaded = settings.load("/server.settings")
...
---@diagnostic disable-next-line: undefined-field
    ...
    TextBox{parent=display,v=1,text="Server Configurator",alignment=CENTER,fg_bg=style.header}
    ...
    TextBox{parent=main_page,x=2,y=2,height=2,text="Welcome to the Server configurator! Please select one of the following options."}

    if tool_ctl.ask_config then
        TextBox{parent=main_page,x=2,y=y_start,height=4,width=49,text="Notice: This device is not configured for this version of the server. If you previously had a valid config, it's not lost. You may want to check the Change Log to see what changed.",fg_bg=cpair(colors.red,colors.lightGray)}
        y_start = y_start + 5
    end

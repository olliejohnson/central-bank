...
local function read_local_manifest()
    ...
    if imfile ~= nil then
        local_ok, local_manifest = pcall(function () return textutils.unserialiseJSON(imfile.readAll()) end)
        imfile.close()
    end
...
println("-- Central Bank Install Manager (Off-Line) --")

if #opts == 0 or opts[1] == "help" then
    println("usage: cbin <mode>")
    println("<mode>")
    lgray()
    println(" check     - check your installed version")
...
elseif mode == "update-rm" or mode == "uninstall" then
    ...
    if not ok then
        red();println("Error parsing loacal installation manifest.");white()
        return
    end

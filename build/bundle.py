...
cbin_file = open("./build/cbin.lua", "r")
cbin_script = cbin_file.read()
cbin_file.close()
...
for app in [ "server" ]:
    ...
    body_a = body_a + f"""
-- install manifest JSON and offlien installer
local install_manifest = "{base64.b64encode(bytes(install_manifest, 'UTF-8')).decode("ASCII")}"
local cbin_offline = "{base64.b64encode(bytes(cbin_script, "UTF-8")).decode("ASCII")}"
